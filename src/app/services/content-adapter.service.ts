...
export interface LabelContent { label: string; placeholder?: string; help?: string }
...
export interface RateFieldsContent {
  ...
  exciseRules: BuilderField<Record<string, Array<{ minCc?: number; maxCc?: number; rate?: number }>>>;
  deMinimis: BuilderField<{ cifThreshold?: number }>;
}
export interface FaqItem { q: string; a: string }
export interface CtasContent { primaryText: string; primaryHref: string; contactText: string; contactHref: string }
...
const DEFAULTS: VehicleCalculatorContentModel = {
  hero: {
    title: { value: 'Vehicle Cost & Import Tax Calculator', review_required: false },
    subtitle: { value: 'Estimate duties, VAT, levies and total landed cost for imported vehicles in Sri Lanka.', review_required: false },
    imageUrl: { value: '', review_required: false },
  },
  intro: {
    body: { value: 'This tool provides an estimate for guidance only. For binding rulings contact the Customs ICT Directorate.', review_required: true },
  ...
  rates: {
    ...
    exciseRules: { value: {
      private_car: [ { maxCc: 1500, rate: 0.05 }, { minCc: 1501, rate: 0.1 } ],
  ...
  faq: [
    { q: 'Is this a legal determination?', a: 'No. This is for guidance only. Consult Customs for a binding ruling.' },
    { q: 'What is CIF?', a: 'Cost, Insurance, and Freight.' },
  ],
  ctas: { primaryText: 'Start Calculation', primaryHref: '#calculator', contactText: 'Contact Customs', contactHref: '/contact' },
};
...
export class ContentAdapterService {
  ...
  load(): VehicleCalculatorContentModel {
    ...
    const injected = globalAny?.BUILDER_VEHICLE_CALC_CONTENT as VehicleCalculatorContentModel | undefined;
