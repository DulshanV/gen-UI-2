...
@Component({
  ...
  template: `
    ...
    >
      <div>
        <label for="currency" class="block text-sm font-medium text-gov-dark">{{
        ...
        <p class="text-xs text-gray-600" id="form-help">
          {{ labels.currency.help }}
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label
            for="declaredPrice"
            class="block text-sm font-medium text-gov-dark"
            >Declared Price</label
          >
          <input
            id="declaredPrice"
            type="number"
        ...
        <div>
          <label
            for="exchangeRate"
            class="block text-sm font-medium text-gov-dark"
            >{{ labels.exchangeRate.label }}</label
          >
          <input
            id="exchangeRate"
            type="number"
            ...
            class="mt-1 w-full border-gray-300 rounded-lg p-2"
            [placeholder]="labels.exchangeRate.placeholder"
          />
          <span
            *ngIf="
              form.controls['exchangeRate'].invalid &&
              form.controls['exchangeRate'].touched
            "
            class="text-xs text-gov-alert"
            >{{ labels.exchangeRate.help }}</span
          >
        </div>
      </div>
      ...
      <div>
        <label
          for="cifOverrideLkr"
          class="block text-sm font-medium text-gov-dark"
          >{{ labels.cif.label }}</label
        >
        <input
          id="cifOverrideLkr"
          type="number"
          step="0.01"
          formControlName="cifOverrideLkr"
          class="mt-1 w-full border-gray-300 rounded-lg p-2"
          [placeholder]="labels.cif.placeholder"
        />
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            class="mt-1 w-full border-gray-300 rounded-lg p-2"
            [placeholder]="labels.freight.placeholder"
          />
        </div>
        <div>
          <label
          ...
          <input
            ...
            class="mt-1 w-full border-gray-300 rounded-lg p-2"
            [placeholder]="labels.insurance.placeholder"
          />
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            class="mt-1 w-full border-gray-300 rounded-lg p-2"
            [placeholder]="labels.engineCc.placeholder"
            required
          />
        </div>
        <div>
          ...
          <input
            ...
            class="mt-1 w-full border-gray-300 rounded-lg p-2"
            [placeholder]="labels.year.placeholder"
          />
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label
            for="vehicleType"
            class="block text-sm font-medium text-gov-dark"
            >{{ labels.vehicleType.label }}</label
          >
          <select
            id="vehicleType"
        ...
        <div class="flex items-center gap-2 mt-6">
          <input
            ...
            class="h-4 w-4"
          />
          <label for="isUsed" class="text-sm font-medium text-gov-dark">{{
            labels.isUsed.label
          }}</label>
        </div>
      </div>
      ...
      <fieldset class="border border-gray-200 rounded-lg p-4">
        <legend class="text-sm font-semibold text-gov-dark">
          {{ labels.fees.label }}
        </legend>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label for="portHandling" class="block text-sm"
              >Port Handling</label
            >
            <input
              id="portHandling"
              type="number"
              step="0.01"
              formControlName="portHandling"
              class="mt-1 w-full border-gray-300 rounded-lg p-2"
            />
          </div>
          <div>
            <label for="extraInsurance" class="block text-sm"
              >Extra Insurance</label
            >
            <input
              id="extraInsurance"
              type="number"
              step="0.01"
              formControlName="extraInsurance"
              class="mt-1 w-full border-gray-300 rounded-lg p-2"
            />
          </div>
          <div>
      ...
      <div class="flex gap-3">
        ...
        <button
          type="button"
          (click)="reset()"
          class="bg-white border border-gray-300 text-gov-dark px-4 py-2 rounded font-semibold"
        >
          Reset
        </button>
      </div>
...
export class CalculatorFormComponent implements OnChanges {
  ...
  submit(): void {
    ...
    if (
      (!input.cifOverrideLkr || input.cifOverrideLkr <= 0) &&
      (!input.declaredPrice || !input.exchangeRate || input.exchangeRate <= 0)
    ) {
      this.error =
        "Enter CIF or provide declared price with a valid exchange rate.";
      return;
    }
